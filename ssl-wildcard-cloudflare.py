...
class SslWildcardCloudflare:
    ...
    def certbot(self):
        ...
        for domain in self.domains:
            ...
            if "No certificates found" in result.stdout:
                ...
                # Si existe el certificado, renovar
                  command = [
                    "certbot", "renew", "--authenticator", "dns-cloudflare",
                    "--dns-cloudflare-credentials", "cloudflare.ini",
                    "--dns-cloudflare-propagation-seconds", "60",
                    "-d", domain, "-d", f"*.{domain}"
                  ]
            ...
            print(f"{'Solicitando nuevo certificado' if 'No certificates found' in result.stdout else 'Renovando certificado'} para {domain}")
    ...
    def check_cert_expiry(self, domain):
        ...
        if "No certificates found" in result.stdout:
            return True  # Necesita renovación si no existe certificado
        ...
        return True  # Por seguridad, renueva si no puede determinar la fecha
    ...
    def backup_certs(self, domain):
        ...
        try:
            ...
            for src_name, dst_name in cert_files:
                src_path = f"/etc/letsencrypt/archive/{domain}/{src_name}"
                dst_path = os.path.join(backup_dir, dst_name)
                
                if os.path.exists(src_path):
                    subprocess.run(['cp', '-p', src_path, dst_path], check=True)
                    print(f"Respaldo creado: {dst_path}")
            return True
    ...
    def copy_certs(self):
        ...
        for domain in self.domains:
            try:
                cmds = [ 
                    f"cp -r /etc/letsencrypt/archive/{domain}/cert.pem {self.cfg['dest_path']}/{domain}.cert.pem",
                    f"cp -r /etc/letsencrypt/archive/{domain}/chain.pem {self.cfg['dest_path']}/{domain}.chain.pem",
                    f"cp -r /etc/letsencrypt/archive/{domain}/fullchain.pem {self.cfg['dest_path']}/{domain}.fullchain.pem",
                    f"cp -r /etc/letsencrypt/archive/{domain}/privkey.pem {self.cfg['dest_path']}/{domain}.key"
                ]
                for cmd in cmds:
    ...
    def parse_arguments():
        ...
        parser = argparse.ArgumentParser(description="Generar certificado SSL con Let's Encrypt y Cloudflare")
...
if __name__ == "__main__":
    ssl = SslWildcardCloudflare("config.yml")
    ssl.run()
